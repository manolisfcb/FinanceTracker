...
class PortfolioModel(db.Model):
    ...
    asset = db.relationship('Asset', backref='portfolios', lazy=True)
    user = db.relationship('UserModel', backref='portfolios', lazy=True)


    def __repr__(self):
        return f'<PortfolioModel(user_id={self.user_id}, asset_id={self.asset_id}, avg_price={self.avg_price}, actual_price={self.actual_price}, adquisition_cost={self.adquisition_cost}, quantity={self.quantity}, profit={self.profit}, percent_return={self.percent_return}, dividend_amount={self.dividend_amount})>'
