(() => {
  const roots = [...document.querySelectorAll('[data-asset-autocomplete]')];
  ...
  roots.forEach((root, rootIndex) => {
    ...
    const excludedIds = new Set(
      (root.dataset.excludedAssetIds || '')
        .split(',')
        .map(value => Number(value.trim()))
        .filter(Boolean),
    );
    if (!input || !selectedId || !menu || !searchUrl) return;

    const optionPrefix = `asset-option-${rootIndex}`;
    let results = [];
    let activeIndex = -1;
    let debounceTimer;
    ...
    const choose = asset => {
      ...
      root.dispatchEvent(new CustomEvent('asset:selected', {
        bubbles: true,
        detail: { asset },
      }));
      closeMenu();
    ...
    const render = assets => {
      results = assets.filter(asset => !excludedIds.has(Number(asset.id)));
      activeIndex = -1;
      ...
      if (!results.length) {
        ...
        empty.textContent = excludedIds.size
        ...
        results.forEach((asset, index) => {
          ...
          option.type = 'button';
          option.id = `${optionPrefix}-${asset.id}`;
    ...
    root.addEventListener('asset-picker:reset', reset);
  });
})();
